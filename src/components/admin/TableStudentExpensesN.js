...
const TableStudentExpensesN = () => {
  const { role, refreshBe, isBlur } = useSelector((state) => state.app);
  ...
  const handleEdit = () => {
    if (itemSelect) {
      dispatch(actions.getDataEditStudentExp(itemSelect));
      dispatch(actions.checkBlur(true));
      dispatch(actions.changeTypeBlur(14));
    }
  };
...
export default TableStudentExpensesN;
