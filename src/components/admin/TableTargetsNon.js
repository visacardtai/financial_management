...
const TableTargetsNon = () => {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      try {
        const response = await apis.apiGetTargetsByStatus(axiosPrivate, 0);
  ...
  const handlDelete = () => {
    if (selected.length !== 0) {
      ...
      dispatch(actions.changeTypeDelete(3));
  ...
  const handleAdd = () => {
    dispatch(actions.changeTypeBlur(4));
    dispatch(actions.checkBlur(!isBlur));
  };
  ...
  const handleCensor = () => {
    if (selected.length !== 0) {
      dispatch(actions.changeTypeBlur(1));
      dispatch(actions.changeTypeDelete(11));
      dispatch(actions.checkBlur(!isBlur));
      dispatch(actions.listDelete(selected));
    } else {
      console.log("Non");
...
export default TableTargetsNon;
