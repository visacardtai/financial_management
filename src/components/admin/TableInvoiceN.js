...
const TableInvoiceN = () => {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      try {
        const response = await apis.apiGetAllInvoice(axiosPrivate, 0, 1);
  ...
  const handleEdit = () => {
    if (itemSelect) {
      dispatch(actions.getDataEditInvoice(itemSelect));
      dispatch(actions.checkBlur(true));
      dispatch(actions.changeTypeBlur(28));
    }
  };
