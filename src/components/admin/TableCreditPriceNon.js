...
const TableCreditPriceNon = () => {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      try {
        const response = await apis.apiGetCreditPriceByStatus(axiosPrivate, 0);
  ...
  const visibleRows = React.useMemo(
    () =>
      stableSort(creditPrice, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage
      ),
    [order, orderBy, page, rowsPerPage, creditPrice]
  );

  const handlDelete = () => {
    if (selected.length !== 0) {
      dispatch(actions.changeTypeBlur(1));
      dispatch(actions.changeTypeDelete(4));
  ...
  const handleAdd = () => {
    dispatch(actions.changeTypeBlur(0));
    dispatch(actions.checkBlur(!isBlur));
  };
  ...
  const handleCensor = () => {
    if (selected.length !== 0) {
      dispatch(actions.changeTypeBlur(1));
      dispatch(actions.changeTypeDelete(8));
      dispatch(actions.checkBlur(!isBlur));
      dispatch(actions.listDelete(selected));
    } else {
      console.log("Non");
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="left">
                          {helpFn.convertDateFormat(row.date_create)}
                        </TableCell>
                        <TableCell align="left">
                          {row.type === true ? "Học lại" : "Học lần đầu"}
                        </TableCell>
                        <TableCell align="left">
...
export default TableCreditPriceNon;
